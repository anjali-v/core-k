...
static inline void set_cs(int val)
{
	gpio_set_value(16, !val);
}
					...
					struct spi_transfer *tfr)
{
	...
	while (bs->rx_len) {
		...
		if (bs->rx_len && time_after(jiffies, timeout)) {
			printk(KERN_ERR "timeout period reached: jiffies: %lu remaining tx/rx: %d/%d.\n", 
...
static int dummy_bcm2835aux_spi_transfer_one(struct spi_master *master,
				       struct spi_device *spi,
				       struct spi_transfer *tfr)
{
	printk(KERN_INFO "%s\n", __FUNCTION__);
...
/* this function gets called from our kernel thread */
int bcm2835aux_spi_transfer_one(struct spi_master *master,
				       struct spi_device *spi,
				       struct spi_transfer *tfr)
{
	struct bcm2835aux_spi *bs = spi_master_get_devdata(master);
...
static int dummy_bcm2835aux_spi_prepare_message(struct spi_master *master,
...
int bcm2835aux_spi_prepare_message(struct spi_master *master,
					  struct spi_message *msg)
{
...
static int dummy_bcm2835aux_spi_unprepare_message(struct spi_master *master,
					    struct spi_message *msg)
{
	printk(KERN_INFO "%s\n", __FUNCTION__);
	return 0;
...
static void dummy_bcm2835aux_spi_handle_err(struct spi_master *master,
				      struct spi_message *msg)
{
	printk(KERN_INFO "%s\n", __FUNCTION__);
}
...
static int bcm2835aux_spi_probe (struct platform_device *pdev)
{
	...
	master->transfer_one = dummy_bcm2835aux_spi_transfer_one;
	master->handle_err = dummy_bcm2835aux_spi_handle_err;
	master->prepare_message = dummy_bcm2835aux_spi_prepare_message;
	master->unprepare_message = dummy_bcm2835aux_spi_unprepare_message;
...
int bcm2835aux_spi_get_master(struct spi_master ** ppMaster)
{
...
int bcm2835aux_spi_get_spi(struct spi_device ** ppSpi)
{
	if (pSpi) *ppSpi = pSpi;
	else return -1;
	return 0;
...
EXPORT_SYMBOL(bcm2835aux_spi_get_master);
EXPORT_SYMBOL(bcm2835aux_spi_get_spi);
EXPORT_SYMBOL(bcm2835aux_spi_prepare_message);
EXPORT_SYMBOL(bcm2835aux_spi_transfer_one);
